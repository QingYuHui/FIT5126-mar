...
class AutoencoderKL(nn.Module):
    ...
    def __init__(self, embed_dim, ch_mult, use_variational=True, ckpt_path=None, num_res_blocks=2):
        ...
        # 1. 定义 3D 配置
        my_ch = 64              # 权重是 64
        my_z_channels = 4       # 权重是 4
        my_in_channels = 1      # 权重是 1
        # my_ch_mult = (1, 1, 2, 2) # 权重层数配置

        self.encoder = Encoder(
            ch=my_ch,
            ch_mult=ch_mult,
            num_res_blocks=num_res_blocks,
            z_channels=my_z_channels,
            in_channels=my_in_channels, 
            double_z=False) # 3D 版本, in_channels=1
        self.decoder = Decoder(
            ch=my_ch,
            ch_mult=ch_mult,
            num_res_blocks=num_res_blocks,
            z_channels=my_z_channels, 
            out_ch=my_in_channels,
            in_channels=my_in_channels)      # 3D 版本, out_ch=1
    ...
    # 把 RGB 变成 Latent 的过程
    def encode(self, x):
        h = self.encoder(x)           # 1. 跑完整个 Encoder 金字塔
        moments = self.quant_conv(h)  # 2. 适配器：转换成 (Mean, LogVar) 的形状
        ...
        # 2D 版本
        # 它将 moments 切分为 均值和方差。它最重要的功能是实现 重参数化技巧 (Reparameterization Trick)
        # 让采样过程可导，从而可以进行反向传播训练
        # posterior = DiagonalGaussianDistribution(moments) # 3. 构建分布对象
        # 返回的是一个后验分布对象 (posterior)，而不是直接的向量
        ...
        if self.use_variational:
            ...
            # VQGAN: 它是确定性的，不需要把 moments 切分成均值和方差
            # 我们强制告诉 Distribution 类：这是一个确定性的值，不要采样
    ...
    # 把 Latent 变成 RGB 的过程
    def decode(self, z):
        z = self.post_quant_conv(z)  # 1. 适配器：把 Latent 特征变回 Decoder 能接受的形状
        dec = self.decoder(z)        # 2. 跑完整个 Decoder 金字塔
        return dec
